...
case "$ID" in
  debian*|ubuntu*)
    ...
    wget -P "$tmpdir" https://github.com/jesseduffield/lazygit/releases/latest/download/lazygit_"${VERSION}"_linux_"${ARCH}".tar.gz
    ...
    ;;
  arch*)
    sudo pacman --needed --noconfirm -Sy lazygit ;;
  *)
